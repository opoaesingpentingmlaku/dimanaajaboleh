...
<form id="create" action="" method="POST">
	<input type="hidden" name="ids" id="ids" size="5" />
	<label>Title :</label> <br>
	<input type="text" name="title" id="title" size="90" /><br><br>

	<label>City :</label> <br>
	<input type="text" name="city" id="city" size="90" /><br><br>

	<label>Location :</label> <br>
	<input type="text" name="location" id="location" size="90" /><br><br>
...
function insert_img(url){
	tinyMCE.get('description').dom.add(tinyMCE.get('description').getBody(), 'p', {style : 'text-align: center'},'<img src="'+ url +'" width="400" />');
}
	...
	$('#fileupload').fileupload({
		...
		complete: function (result) {
			...
			$.each(res.files, function (index, file) {
				$('#template-download').append('<li id="item_'+file.id+'">'+file.name
					+'<input type="button" name="insert" value="ADD" onclick="insert_img(\''+file.url+'\')" />'
					+'<input type="button" name="delete" value="DELETE" onclick="del_img('+file.id+')" />'
					+'</li>'
				);
