...
class Factory extends CActiveRecord
{
	...
	public function FactoryReview(){
		if ( empty($this->AdvisorFactoryReview))
			$this->AdvisorFactoryReview = AdvisorFactoryReview::model()->find("factory_id = '".$this->id."'");
			
		return $this->AdvisorFactoryReview;
	}
	...
	public function getPhotos()
	{
		...
		$photos = AdvisorReviewPhoto::model()->findAll("reference_code = '".$this->AdvisorFactoryReview->reference_code."'");
	...
	{
		$creator = '';
		if ( !empty($this->user_created)){
			$creator = User::model()->findByPk($this->user_created);
		}
		return $creator;
	}
